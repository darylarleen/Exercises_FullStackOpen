  ...
  if (props.larray > 0) {
    return (
      <div>
        ...
        <p> {props.arreglo[props.indice]} </p>
...
const App = () => {
  ...
  //Highest contains and display the info for the anecdote with highest number of votes
  return (
    <div>
      ...
      <Highest larray={myArray.length} arreglo={anecdotes} indice={myArray.indexOf(Math.max(...myArray))} nvotes={Math.max(...myArray)} />
    </div>
  )
}
